 ...
 * were pixels in input that are in the White range threshold. All black pixels on the mask were
 * pixels in the input that were not in the White range threshold.
 *
 ...
 * been thought to be White by the mask. Since the ring stack will most likely be the largest blob
 * of White in the view of the camera.
 *
...
class UGContourRingPipeline(
    ...
    companion object Config {
        ...
        var lowerWhite = Scalar(0.0, 141.0, 0.0)
        var upperWhite = Scalar(255.0, 230.0, 95.0)
    ...
    override fun processFrame(input: Mat?): Mat {
        ...
        try { // try catch in order for opMode to not crash and force a restart
            ...
            /**checking if any pixel is within the White bounds to make a black and white mask**/
            val mask = Mat(mat.rows(), mat.cols(), CvType.CV_8UC1) // variable to store mask in
            Core.inRange(mat, lowerWhite, upperWhite, mask)
        ...
        /**returns the black and White mask with contours drawn to see logic in action**/
        return ret
    }
}